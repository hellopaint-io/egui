use crate::{
    ...
    CornerRadius, Image, IntoAtoms, NumExt as _, Response, Sense, Stroke, TextStyle, TextWrapMode,
...
impl<'a> Button<'a> {
    ...
    pub fn atom_ui(self, ui: &mut Ui) -> AtomLayoutResponse {
        ...
        // `AtomLayoutStyle::apply` only sets the text style as a *fallback*, and a
        // plain `WidgetText::Text` bakes in `Visuals::override_text_color` before it
        // ever consults one. A theme that owns the whole button asks for its text
        // style to be pushed onto the `Ui` instead, so it also reaches nested
        // `AtomLayout`s and can't be beaten by an app-wide override. Anything that
        // names its own size, family or color still wins.
        let pushed_text_style = force_text_style.then(|| {
            let text_style = &atom_layout_style.text_style;
            let color = ui
                .visuals_mut()
                .override_text_color
