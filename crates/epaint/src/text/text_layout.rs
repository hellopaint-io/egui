...
fn layout_shaped_run(
    ...
    for (info, pos) in iter::zip(glyph_buffer.glyph_infos(), glyph_buffer.glyph_positions()) {
        ...
        let glyph = if glyph_id == skrifa::GlyphId::NOTDEF {
            ...
            // On WASM, if no loaded font actually contains `chr`, pass NOTDEF so
            // `allocate_glyph` renders it through the HTML canvas fallback rather
            // than substituting the replacement character.
            #[cfg(target_arch = "wasm32")]
            let shaped_glyph_id = if font.has_glyph(chr) {
                glyph_info.id.unwrap_or(skrifa::GlyphId::NOTDEF)
            } else {
                skrifa::GlyphId::NOTDEF
            };
            #[cfg(not(target_arch = "wasm32"))]
            let shaped_glyph_id = glyph_info.id.unwrap_or(skrifa::GlyphId::NOTDEF);
